...
- **What it is:** A single continuous trip where the app's visual weight scales with the stakes. **Quiet mode**: a light, minimal card (37 → City Centre, 6 min) that makes no demands on attention. **Alert emerging**: the card gains an amber border and a discreet "Something changed with your bus / See →" strip; present but not interrupting. **Guidance mode**: tapping in flips the screen to dark, showing "Bus 37 delayed — expected in 14 min (was 6), traffic on Prinsens gate", three options (wait here, walk to Kongens gate for Bus 4, wait for Bus 15) and a navigate button. **Resolved**: back to the light, minimal card ("3 min · On its way").
